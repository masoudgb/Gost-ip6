...
# Download the Gost.sh script from the provided GitHub link
wget https://github.com/masoudgb/Gost-ip6/raw/main/Gost.sh

# Create the /etc/gost directory if it doesn't exist
sudo mkdir -p /etc/gost

# Move the downloaded Gost.sh script to /etc/gost
sudo mv Gost.sh /etc/gost/

# Make the script executable
sudo chmod +x /etc/gost/Gost.sh

# Change to the /etc/gost directory
cd /etc/gost || exit

# Execute the Gost.sh script
sudo ./Gost.sh
