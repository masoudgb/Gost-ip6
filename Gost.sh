...
if [ "$port_option" -eq 1 ]; then
    ...
    read -p $'\e[36mPlease enter the port range (e.g., 54,65535): \e[0m' port_range

    IFS=',' read -ra port_array <<< "$port_range"

    # Check if the start and end port values are within the valid range
    if [ "${port_array[0]}" -lt 54 -o "${port_array[1]}" -gt 65535 ]; then
        echo $'\e[33mInvalid port range. Please enter a valid range starting from 54 and up to 65535.\e[0m'
        exit
    fi
...
# If option 3 is selected
    ...
    read -p $'\e[36mPlease enter the new destination (Kharej) IP (separated by commas): \e[0m' destination_ip
    read -p $'\e[36mPlease enter the port: \e[0m' port
    read -p $'\e[32mSelect the protocol:\n\e[0m\e[36m1. \e[0mBy Tcp Protocol \n\e[36m2. \e[0mBy Grpc Protocol \e[32m\nYour choice: \e[0m' protocol_option
...
# If option 5 is selected
    ...
    # Countdown for restarting Gost in a single line
    echo $'\e[32mRestarting Gost in 3 seconds... \e[0m' && sleep 1 && echo $'\e[32m2... \e[0m' && sleep 1 && echo $'\e[32m1... \e[0m' && sleep 1 && { sudo systemctl daemon-reload && sudo systemctl restart gost_*.service && echo $'\e[32mGost restarted successfully.\e[0m'; }
    bash "$0"
